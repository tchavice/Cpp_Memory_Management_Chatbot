...
ChatLogic::ChatLogic()
{
    ...
    //_chatBot.SetChatLogicHandle(this);  //Task 5 _chabot changed from ptr to variable, removed
...
ChatLogic::~ChatLogic()
{
    ...
    //delete _chatBot; //Task 5 - removed constructor/destructor

    // delete all nodes
    // Task 3 - remove deletion of unique ptrs _node
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // Task 3 update node to unique ptr
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // create new edge
                            // Task 4 - convert edge to unique ptr
                            //GraphEdge *edge = new GraphEdge(id);
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id);
                            edge->SetChildNode(childNode->get()); //Task 3 - changed node to unique ptr
                            edge->SetParentNode(parentNode->get()); //Task 3
                            //_edges.push_back(edge); //not used?

                            // find all keywords for current node
                            AddAllTokensToElement("KEYWORD", tokens, *edge);

                            // store reference in child node and parent node
                            (*childNode)->AddEdgeToParentNode(edge.get());  //Task 4 : edge is unique ptr
                            (*parentNode)->AddEdgeToChildNode(std::move(edge));  //Task 4: edge is unique ptr
                        }
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                // Task 3 - changed _node to unique ptr
    ...
    //Task 5 - create local chat bot instance
    ChatBot localChatBot("../images/chatbot.png");  //Task 5 - moved from constructor
    //SetChatbotHandle(&localChatBot);
    localChatBot.SetChatLogicHandle(this); //Task 5 moved from constructor
    localChatBot.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(localChatBot)); //Task 5
